...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/list_markets \
  -H 'Content-Type: application/json' \
  -d '{"limit":25,"open_only":true}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/list_markets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
